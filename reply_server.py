...
# 系统初始化时的默认密码：优先读取环境变量，未设置时由db_manager生成随机初始密码
...
# ===== 前后端分离支持：CORS 配置 =====
# 开发环境 Vite dev server（默认5173）及通过 CORS_ORIGINS 环境变量指定的源允许跨域。
# 生产同源部署时无需 CORS，此配置主要服务于本地前后端分离开发。
try:
    ...
    logger.info(f"CORS已启用，允许来源: {_cors_origins}")
except Exception as _cors_err:
    logger.warning(f"CORS中间件配置失败(不影响系统运行): {_cors_err}")
...
# 前端工程构建产物目录（前后端分离：vite build 出的 SPA）
# 当该目录存在 index.html 时，/ 与 /admin 优先返回新前端；否则回退到旧 static/
PROJECT_ROOT = os.path.dirname(os.path.abspath(__file__))
frontend_dist_dir = os.path.join(PROJECT_ROOT, 'frontend', 'dist')
_SPA_INDEX = os.path.join(frontend_dist_dir, 'index.html')
_SPA_ASSETS = os.path.join(frontend_dist_dir, 'assets')
_FRONTEND_ACTIVE = os.path.exists(_SPA_INDEX)


def _serve_spa() -> Optional[HTMLResponse]:
    """返回新前端 SPA 的 index.html；不存在时返回 None"""
    if _FRONTEND_ACTIVE and os.path.exists(_SPA_INDEX):
...
# 托管 vite 构建产物（新前端）的静态资源
if _FRONTEND_ACTIVE and os.path.exists(_SPA_ASSETS):
    try:
        app.mount('/assets', StaticFiles(directory=_SPA_ASSETS), name='frontend-assets')
        logger.info(f"已挂载前端构建产物静态资源: {_SPA_ASSETS}")
    except Exception as e:
        logger.warning(f"挂载前端资源失败: {e}")
...
# 重定向根路径到登录页面（优先返回新前端 SPA，否则回退旧登录页）
...
# ===== 前端 SPA fallback =====
# 当启用新前端且请求的是非 API、非 /static、非 /assets、非已注册 API 的路径时，
# 返回 index.html，使 Vue Router 的 history 模式刷新子路由不 404。
if _FRONTEND_ACTIVE:
    ...
    async def spa_fallback(request, call_next):
        ...
        # 只对 GET 且非已有资源/API 的路径做 fallback
        if request.method == "GET" and _FRONTEND_ACTIVE:
            if (path.startswith("/static") or path.startswith("/assets")
                    or path.startswith("/api") or path.startswith("/docs")
                    or path.startswith("/redoc") or path.startswith("/openapi.json")
                    or path == "/health" or path == "/" or path.startswith("/login.html")
                    or path.startswith("/register.html") or path == "/admin"):
    ...
    logger.info("前端 SPA fallback 已启用（history 模式刷新不再 404）");
...
# API秘钥（生产环境中应该从环境变量读取，禁止使用固定默认值）
# 注意：优先从系统设置或环境变量中读取QQ回复消息秘钥
import os as _os
API_SECRET_KEY = _os.getenv("XIANYU_API_SECRET_KEY", "")  # 从环境变量读取，无默认值
...
def verify_api_key(api_key: str) -> bool:
    ...
    try:
        ...
        # 如果系统设置和环境中都没有配置，拒绝验证（不再回退到固定默认值）
        if not qq_secret_key:
            qq_secret_key = API_SECRET_KEY
        if not qq_secret_key:
            logger.error("API秘钥未配置（系统设置QQ回复秘钥和环境变量均未设置）")
            return False
...
# 通知渠道的对象级授权辅助函数：校验渠道是否属于当前用户
def _check_channel_owned(current_user: Dict[str, Any], channel_id: int) -> Optional[dict]:
    """校验通知渠道是否属于当前用户，不属于则抛403。返回渠道信息。"""
    from db_manager import db_manager
    user_id = current_user['user_id']
    ...
    # 管理员可管理所有渠道；普通用户只能操作属于自己(user_id)的渠道
    owner_id = channel.get('user_id')
    if not is_admin and owner_id is not None and owner_id != user_id:
        raise HTTPException(status_code=403, detail='无权限操作该通知渠道')
    return channel
...
def get_notification_channel(channel_id: int, current_user: Dict[str, Any] = Depends(get_current_user)):
    """获取指定通知渠道"""
    try:
        channel = _check_channel_owned(current_user, channel_id)
        return channel
    except HTTPException:
        raise
    except Exception as e:
...
def update_notification_channel(channel_id: int, channel_data: NotificationChannelUpdate, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    from db_manager import db_manager
    try:
        _check_channel_owned(current_user, channel_id)
...
def delete_message_notification(notification_id: int, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    from db_manager import db_manager
    try:
        # 校验通知配置所属Cookie是否属于当前用户
        user_id = current_user['user_id']
        is_admin = current_user.get('username') == 'admin'
        if not is_admin:
            notif = db_manager.get_message_notification(notification_id)
            if not notif:
                raise HTTPException(status_code=404, detail='通知配置不存在')
            cid = notif.get('cookie_id')
            user_cookies = db_manager.get_all_cookies(user_id)
            if cid not in user_cookies:
                raise HTTPException(status_code=403, detail="无权限删除该通知配置")
...
def _sanitize_system_settings(settings: Dict[str, Any]) -> Dict[str, Any]:
    """移除系统设置中的敏感字段"""
...
def get_system_settings(_: None = Depends(require_auth)):
    ...
    try:
        ...
        return _sanitize_system_settings(settings)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def update_system_setting(key: str, setting_data: SystemSettingIn, admin_user: Dict[str, Any] = Depends(require_admin)):
    """更新系统设置（仅管理员，且禁止修改敏感字段）"""
    from db_manager import db_manager
    try:
        # 禁止直接修改敏感字段/密码
        if key in SENSITIVE_SETTING_KEYS or key == 'admin_password_hash':
            raise HTTPException(status_code=400, detail='不允许直接修改该敏感设置项')
...
def update_card(card_id: int, card_data: dict, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        from db_manager import db_manager
        # 对象级授权：校验卡券是否属于当前用户
        if not _card_owned_by(current_user, card_id):
            raise HTTPException(status_code=403, detail="无权限操作该卡券")
        # 验证多规格字段
...
async def _card_owned_by(current_user: Dict[str, Any], card_id: int) -> bool:
    """校验卡券是否属于当前用户（管理员放行）"""
    from db_manager import db_manager
    try:
...
async def get_all_items_from_account(request: dict, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        # 对象级授权：校验该cookie是否属于当前用户
        if not _cookie_owned_by(current_user, cookie_id):
            return {"success": False, "message": "无权限访问该账号"}
...
async def _cookie_owned_by(current_user: Dict[str, Any], cookie_id: str) -> bool:
    """校验cookie是否属于当前用户（管理员放行）"""
    from db_manager import db_manager
    try:
        if current_user.get('username') == 'admin':
            return True
