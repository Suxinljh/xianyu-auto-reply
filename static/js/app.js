...
async function handleItemSearch(event) {
    ...
    try {
        ...
        sessionChecker = setInterval(async () => {
            ...
            try {
                checkCount++;
                const checkResponse = await fetch('/api/captcha/sessions', { headers: { 'Authorization': 'Bearer ' + (localStorage.getItem('auth_token') || '') } });
                            ...
                            <div class="row mb-4">
                                <div class="col-md-3">
                                    <div class="text-center p-3 bg-light rounded">
                                        <div class="h2 text-primary mb-1">${escapeHtml(data.total_users) || 0}</div>
...
async function showUpdateInfo(newVersion) {
    ...
    if (updateInfo.updates && updateInfo.updates.length > 0) {
        updateList = updateInfo.updates.map(item => `<li class="mb-2">${escapeHtml(item)}</li>`).join('');
    }
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <p class="fs-4 text-success fw-bold">${escapeHtml(updateInfo.version)}</p>
                            </div>
                            <div class="col-md-6">
                                <h6><i class="bi bi-calendar me-1"></i>发布日期</h6>
                                <p class="text-muted">${escapeHtml(updateInfo.releaseDate || '未知')}</p>
                            </div>
                        </div>
                        <hr>
...
function startCaptchaSessionMonitor() {
    ...
    captchaSessionMonitor = setInterval(async () => {
        try {
            checkCount++;
            const response = await fetch('/api/captcha/sessions', { headers: { 'Authorization': 'Bearer ' + (localStorage.getItem('auth_token') || '') } });
            const data = await response.json();
