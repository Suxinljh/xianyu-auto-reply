...
class ImageManager:
    ...
    def _resolve_within_upload_dir(self, image_path: str) -> Optional[str]:
        """将图片路径解析为上传目录内的绝对路径。

        防止路径穿越（path traversal）：解析真实路径后必须位于 upload_dir 内，
        否则返回 None。
        """
        upload_abs = os.path.realpath(self.upload_dir)
        # 与上传目录拼接（目录本身不参与对外路径拼接）
        full_path = os.path.realpath(os.path.join(upload_abs, os.path.basename(image_path)))
        if full_path.startswith(upload_abs + os.sep) or full_path == upload_abs:
            return full_path
        return None
    ...
    def delete_image(self, image_path: str) -> bool:
        ...
        """
        try:
            # 安全解析路径，仅允许删除上传目录内的文件
            full_path = self._resolve_within_upload_dir(image_path)
            if not full_path:
                logger.warning(f"非法或越界的图片路径，拒绝删除: {image_path}")
                return False
    ...
    def get_image_info(self, image_path: str) -> Optional[dict]:
        ...
        """
        try:
            # 安全解析路径，仅允许读取上传目录内的文件
            full_path = self._resolve_within_upload_dir(image_path)
            if not full_path:
                logger.warning(f"非法或越界的图片路径: {image_path}")
                return None
