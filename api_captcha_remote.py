...
def _is_valid_token(token: str) -> bool:
    """检验Token是否有效（供HTTP依赖和WebSocket共用）"""
    if not token:
        return False
    try:
        # 延迟导入 reply_server，避免循环导入
        import reply_server
        session_tokens = reply_server.SESSION_TOKENS
        token_expire = reply_server.TOKEN_EXPIRE_TIME
    except Exception:
        return False
    ...
    if not token_data:
        return False
    if time.time() - token_data['timestamp'] > token_expire:
        session_tokens.pop(token, None)
        return False
    return True


def require_captcha_auth(authorization: Optional[str] = Header(default=None)):
    """刮刮乐远程控制的认证依赖：验证Bearer Token是否有效。
    所有远程控制端点都必须经过认证，防止攻击者劫持验证码控制页面。"""
    ...
    try:
        import reply_server
        return reply_server.SESSION_TOKENS.get(token)
    except Exception:
...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    安全要求：通过查询参数 token 校验登录态，未认证则拒绝连接。
    """
    await websocket.accept()

    # 安全：校验认证 token（来自查询参数 token）
    token = websocket.query_params.get("token", "")
    if not _is_valid_token(token):
        await websocket.send_json({'type': 'error', 'message': '未授权访问，认证失败'})
...
@router.get("/sessions")
async def get_active_sessions(_: dict = Depends(require_captcha_auth)):
    """获取所有活跃的验证会话"""
    sessions = []
...
async def get_session_info(session_id: str, _: dict = Depends(require_captcha_auth)):
    """获取指定会话的信息"""
...
async def get_screenshot(session_id: str, _: dict = Depends(require_captcha_auth)):
    """获取最新截图"""
    screenshot = await captcha_controller.update_screenshot(session_id)
...
async def handle_mouse_event(event: MouseEvent, _: dict = Depends(require_captcha_auth)):
...
async def check_completion(request: SessionCheckRequest, _: dict = Depends(require_captcha_auth)):
    """检查验证是否完成"""
...
async def close_session(session_id: str, _: dict = Depends(require_captcha_auth)):
    """关闭会话"""
...
async def get_captcha_status(session_id: str, _: dict = Depends(require_captcha_auth)):
    """
    获取验证状态
    用于前端轮询检查验证是否完成
    """
