...
class DBManager:
    ...
    def __init__(self, db_path: str = None):
        ...
        # Cookie等敏感字段的落库加密配置
        # 加密密钥优先级：环境变量 COOKIE_ENCRYPTION_KEY > 持久化密钥文件 > 自动生成并持久化
        # 默认启用落库加密（自动生成并持久化密钥），防止数据库中明文保存Cookie/密码
        self._crypto_fernet = None
        self._crypto_enabled = False
        self.__crypto_key_file = None
        self._init_crypto()
        self.init_db()
    ...
    def _load_or_create_crypto_key(self) -> Optional[str]:
        """从持久化密钥文件加载或生成新的Fernet密钥

        密钥文件保存在数据库同级目录下（.crypto_key），权限设置为600。
        生成一次后复用，避免重启后密钥变化导致历史密文无法解密。
        """
        try:
            from cryptography.fernet import Fernet
        except Exception as e:
            logger.error(f"cryptography 库不可用，无法启用落库加密: {e}")
            return None

        data_dir = self._resolve_data_dir() or '.'
        key_file = os.path.join(data_dir, '.crypto_key')
        self.__crypto_key_file = key_file
        ...
        try:
            if os.path.exists(key_file):
                ...
                if key:
                    ...
                    logger.info(f"已从密钥文件加载加密密钥: {key_file}")
                    return key
        except Exception as e:
        ...
        try:
            ...
            try:
                ...
                pass
            logger.info(f"已生成新的加密密钥并保存到: {key_file}（权限600）")
            return new_key
        except Exception as e:
            logger.error(f"生成并保存加密密钥失败: {e}")
            return None

    def _init_crypto(self):
        """初始化Fernet加密（用于Cookie/账号密码落库加密）

        密钥优先级：
          1. 环境变量 COOKIE_ENCRYPTION_KEY（手动指定）
          2. 持久化密钥文件（数据库同级目录 .crypto_key）
          3. 自动生成新密钥并持久化
        默认启用落库加密。
        如需完全关闭加密，可设置环境变量 COOKIE_ENCRYPTION_DISABLED=true。
        """
        # 允许显式关闭加密（默认开启）
        if os.getenv('COOKIE_ENCRYPTION_DISABLED', 'false').lower() == 'true':
            logger.warning("已通过 COOKIE_ENCRYPTION_DISABLED 关闭落库加密")
            self._crypto_enabled = False
            self._crypto_fernet = None
            return

        # 1) 环境变量优先
        key = (os.getenv('COOKIE_ENCRYPTION_KEY') or '').strip()
        source = "环境变量COOKIE_ENCRYPTION_KEY"
        if not key:
            # 2) 持久化密钥文件 或 自动生成
            key = self._load_or_create_crypto_key()
            source = "持久化密钥文件/自动生成"

        if not key:
            logger.error("无法获取加密密钥，将不使用落库加密")
            self._crypto_enabled = False
            self._crypto_fernet = None
            return
        ...
        try:
            from cryptography.fernet import Fernet
            self._crypto_fernet = Fernet(key.encode('utf-8'))
            self._crypto_enabled = True
            logger.info(f"已启用Cookie/账号密码落库加密（密钥来源: {source}）")
        except Exception as e:
            logger.error(f"初始化加密失败（密钥无效），将不使用落库加密: {e}")
            self._crypto_enabled = False
            self._crypto_fernet = None

    def encrypt_secret(self, plaintext: str) -> str:
        """加密敏感字段。未启用时原样返回。"""
        if not self._crypto_enabled or not self._crypto_fernet:
            return plaintext
        try:
            if plaintext:
                return "enc::" + self._crypto_fernet.encrypt(plaintext.encode('utf-8')).decode('utf-8')
            return plaintext
        except Exception as e:
    ...
    def decrypt_secret(self, ciphertext: str) -> Optional[str]:
        """解密敏感字段。未启用或非加密值时原样返回；解密失败返回None。"""
        if not ciphertext:
            return ciphertext
        if self._crypto_enabled and self._crypto_fernet and ciphertext.startswith("enc::"):
            try:
                return self._crypto_fernet.decrypt(ciphertext[len("enc::"):].encode('utf-8')).decode('utf-8')
            except Exception as e:
                logger.error(f"敏感字段解密失败（可能是旧数据或密钥不匹配: {e}")
                return None
        return ciphertext
    ...
    def init_db(self):
        ...
        try:
            ...
            ('qq_reply_secret_key', '', 'QQ回复消息API秘钥（留空则需通过环境变量XIANYU_API_SECRET_KEY配置）')
            ''')
    ...
    def update_admin_user_id(self, cursor):
        ...
        try:
            ...
            if not admin_exists:
                # 首次创建admin用户：优先读取环境变量INITIAL_ADMIN_PASSWORD，
                # 未设置时生成随机初始密码，避免使用固定默认密码
                import os as _os, string as _string, random as _random
                env_password = _os.getenv("INITIAL_ADMIN_PASSWORD", "").strip()
                if env_password:
                    initial_password = env_password
                else:
                    _chars = _string.ascii_letters + _string.digits
                    initial_password = ''.join(_random.choices(_chars, k=16))
                default_password_hash = self.hash_password(initial_password)
                cursor.execute('''
                INSERT INTO users (username, email, password_hash) VALUES
                ('admin', 'admin@localhost', ?)
                ''', (default_password_hash,))
                logger.warning(f"已创建默认admin用户。初始密码: {initial_password} （请尽快登录并修改密码）")
    ...
    SENSITIVE_SQL_KEYWORDS = ('password', 'passwd', 'secret', 'token', 'api_key', 'apikey')
    ...
    def _log_sql(self, sql: str, params: tuple = None, operation: str = "EXECUTE"):
        ...
        # 判断SQL是否涉及敏感字段，涉及则完全隐藏参数值
        sql_lower = sql.lower()
        sensitive_sql = any(kw in sql_lower for kw in self.SENSITIVE_SQL_KEYWORDS)
        ...
        with self.lock:
            try:
                ...
                self._execute_sql(cursor,
                    ...
                    (cookie_id, self.encrypt_secret(cookie_value), user_id)
                )
    ...
    def get_cookie(self, cookie_id: str) -> Optional[str]:
        ...
        with self.lock:
            try:
                ...
                return self.decrypt_secret(result[0]) if result else None
            except Exception as e:
        ...
        with self.lock:
            try:
                ...
                for row in cursor.fetchall():
                    if not row[1]:
                        continue
                    _dec = self.decrypt_secret(row[1])
                    if _dec is not None:
                        result[row[0]] = _dec
                return result
            except Exception as e:
    ...
    def get_cookie_by_id(self, cookie_id: str) -> Optional[Dict[str, str]]:
        ...
        with self.lock:
            try:
                ...
                result = cursor.fetchone()
                if result:
                    _decrypted = self.decrypt_secret(result[1])
                    return {
                        'id': result[0],
                        'cookies_str': _decrypted,  # 使用cookies_str字段名以匹配调用方期望
                        'value': _decrypted,        # 保持向后兼容
                        'created_at': result[2]
                    }
                return None
            except Exception as e:
        ...
        with self.lock:
            try:
                ...
                if result:
                    return {
                        'id': result[0],
                        'value': self.decrypt_secret(result[1]),
                        'user_id': result[2],
                        'auto_confirm': bool(result[3]),
                        'remark': result[4] or '',
                        'pause_duration': result[5] if result[5] is not None else 10,  # 0是有效值，表示不暂停
                        'username': self.decrypt_secret(result[6] or ''),
                        'password': self.decrypt_secret(result[7] or ''),
        ...
        with self.lock:
            try:
                ...
                if not exists:
                    ...
                    insert_values = [cookie_id, self.encrypt_secret(cookie_value), user_id]
                    insert_placeholders = ['?', '?', '?']
                    
                    if username is not None:
                        insert_fields.append('username')
                        insert_values.append(self.encrypt_secret(username))
                        insert_placeholders.append('?')
                    
                    if password is not None:
                        insert_fields.append('password')
                        insert_values.append(self.encrypt_secret(password))
                        insert_placeholders.append('?')
                    ...
                    if cookie_value is not None:
                        ...
                        params.append(self.encrypt_secret(cookie_value))
                    
                    if username is not None:
                        update_fields.append("username = ?")
                        params.append(self.encrypt_secret(username))
                    
                    if password is not None:
                        update_fields.append("password = ?")
                        params.append(self.encrypt_secret(password))
    ...
    def get_message_notification(self, notification_id: int) -> Optional[Dict[str, any]]:
        ...
        with self.lock:
            try:
                cursor = self.conn.cursor()
                cursor.execute('''
                SELECT id, cookie_id, channel_id, enabled, created_at, updated_at
                FROM message_notifications WHERE id = ?
                ''', (notification_id,))
                row = cursor.fetchone()
                if row:
                    return {
    ...
    def hash_password(password: str, salt: str = None) -> str:
        """使用带盐的PBKDF2-SHA256对密码进行哈希，返回格式: pbkdf2$iterations$salt$hash"""
        if salt is None:
            salt = secrets.token_hex(16)
        iterations = 100000
        digest = hashlib.pbkdf2_hmac('sha256', password.encode('utf-8'), salt.encode('utf-8'), iterations)
        return "pbkdf2$" + str(iterations) + "$" + salt + "$" + digest.hex()

    @staticmethod
    def verify_password(password: str, stored_hash: str) -> bool:
        """验证密码。兼容旧的sha256无盐格式和新的pbkdf2带盐格式。"""
        if not stored_hash:
            return False
        # 兼容旧的SHA-256无盐哈希（用于历史数据迁移）
    ...
    def verify_user_password(self, username: str, password: str) -> bool:
        """验证用户密码，兼容旧的无盐SHA256哈希和新PBKDF2带盐哈希"""
        user = self.get_user_by_username(username)
        if not user:
            return False

        return self.verify_password(password, user['password_hash']) and user['is_active']
