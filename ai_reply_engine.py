...
注意：结合商品信息，给出实用建议。'''
    ...
    def _call_dashscope_api(self, settings: dict, messages: list, max_tokens: int = 100, temperature: float = 0.7) -> str:
        ...
        logger.info(f"发送的prompt长度: {len(prompt)} 字符") # 避免泄露完整prompt
        ...
        logger.debug(f"DashScope API响应: {resp_text[:200]}...")
    ...
    def _call_gemini_api(self, settings: dict, messages: list, max_tokens: int = 100, temperature: float = 0.7) -> str:
        ...
        if not user_content:
            logger.warning(f"Gemini API 调用: 未在消息中找到 'user' 角色内容（出于安全考虑不回显完整消息）")
        ...
        logger.debug(f"Gemini Payload: {payload_str[:200]}...")
        ...
        logger.debug(f"Gemini API 响应: {resp_str[:200]}...")
...
请根据以上信息生成回复："""
                ...
                if self._is_dashscope_api(settings):
                    ...
                    if not client:
                        return None
                    logger.info(f"发送AI消息条数: {len(messages) if isinstance(messages, list) else 0}（出于安全考虑不回显内容）")
                ...
                logger.info(f"AI回复生成成功 (账号: {cookie_id}): {(reply[:100] + '...') if reply and len(reply) > 100 else reply}")
                return reply
