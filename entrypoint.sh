...
# 设置目录权限
echo "设置目录权限..."
chmod 750 /app/data /app/logs /app/backups /app/static/uploads /app/static/uploads/images
chmod 750 /app/trajectory_history 2>/dev/null || true
# 加密密钥文件权限收紧为600
if [ -f "/app/data/.crypto_key" ]; then chmod 600 /app/data/.crypto_key; fi
echo "✓ 权限设置完成"
